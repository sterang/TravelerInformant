...
$mysqli = new mysqli($host, $user, $pw, $db);
	...
	$sql = "SELECT avg(humedad) as count FROM datos WHERE id_mod=1 AND fecha BETWEEN DATE_SUB(NOW(), INTERVAL 1 MONTH) AND NOW()";
    $viewer = mysqli_query($mysqli,$sql);
	$viewer = mysqli_fetch_all($viewer,MYSQLI_ASSOC);
	$viewer = json_encode(array_column($viewer, 'count'),JSON_NUMERIC_CHECK);

	/* Getting demo_click table data */
	$sql = "SELECT avg(humedad) as count FROM datos WHERE id_mod=1 AND fecha BETWEEN DATE_SUB(NOW(), INTERVAL 2 MONTH) AND NOW()";
...
$(function () { 
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Humedad promedio por meses'
        },
        xAxis: {
